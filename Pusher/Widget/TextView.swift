...
struct TextView: NSViewRepresentable {
    ...
    init(text: String, isEditable: Bool = false) {
        self.init(text: Binding<String>.constant(text), isEditable: false)
    }
    
    func makeNSView(context: Context) -> NSScrollView {
        let text = NSTextView()
        text.backgroundColor = NSColor(red: 246.0/255.0, green: 246.0/255.0, blue: 246.0/255.0, alpha: 1.0)
